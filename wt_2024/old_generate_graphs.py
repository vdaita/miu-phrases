...
def graph_keywords(keywords, keyword_name):
    df = pd.read_csv("../sp_2024/miu-website/company_website_second_round_with_additional_firms.csv", low_memory=False)
    print("Opened CSV")
    ...
    # %%
    results = [generate_final_value_by_year((keyword, term_count[keyword])) for keyword in tqdm(keywords)]
...
if __name__ == "__main__":
    keywords_file = "generated_words_reference.json"
    import json
    with open(keywords_file) as f:
        keywords = json.load(f)
    for keyword_name, keyword_list in tqdm(keywords.items()):
        graph_keywords(keyword_list, keyword_name)